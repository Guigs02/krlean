...
#ifndef KRLEAN_VER
#define KRLEAN_VER (OS_MAJ_VERS * 10000 + OS_MIN_VERS * 100 + OS_RELEASE)
#endif
...
// Signals
//
// Some of these signals are not used in krlean, but are included
// for completeness.
//
...
size);
osapi int pread(handle_t
f,
void *data, size_t
size,
off64_t offset
);
osapi int pwrite(handle_t
f,
const void *data, size_t
size,
off64_t offset
);
osapi int ioctl(handle_t
f,
int cmd,
const void *data, size_t
size);
osapi int readv(handle_t
f,
const struct iovec *iov,
int count
);
osapi int writev(handle_t
f,
const struct iovec *iov,
int count
);
...
osapi int fstat(handle_t
f,
struct stat *buffer
);
osapi int fstat64(handle_t
f,
...
_opendir(const char *name);

osapi int _readdir(handle_t
f,
...
osapi int waitone(handle_t
h,
int timeout
);
osapi int waitall(handle_t * h, int
count,
int timeout
);
osapi int waitany(handle_t * h, int
count,
int timeout
);
...
mkevent(int manual_reset, int initial_state);

osapi int epulse(handle_t
h);
osapi int eset(handle_t
h);
osapi int ereset(handle_t
h);

osapi handle_t

mksem(int initial_count);

osapi int semrel(handle_t
h,
int count
);
...
beginthread(void (__stdcall *startaddr)(void *), unsigned int stacksize, void *arg, int flags, char *name,
            struct tib **ptib);

osapi int suspend(handle_t
thread);
osapi int resume(handle_t
thread);
osapi struct tib *getthreadblock(handle_t
thread);
osapi handle_t
getprochandle(pid_t
pid);
...
osapi int getchildstat(pid_t
pid,
int *status
);
osapi int setchildstat(pid_t
pid,
int status
);
osapi int setcontext(handle_t
thread,
void *context
);
osapi int getcontext(handle_t
thread,
void *context
);
osapi int getprio(handle_t
thread);
osapi int setprio(handle_t
thread,
int priority
);
osapi clock_t
...
osapi int kill(pid_t
pid,
int signum
);
osapi int sendsig(handle_t
thread,
int signum
);
...
osapi void *_lmalloc(size_t
size);

osapi void *_lrealloc(void *mem, size_t size);

osapi void *_lcalloc(size_t
num,
size_t size
);
...
#else
osapi void *malloc(size_t
size);

osapi void *realloc(void *mem, size_t size);

osapi void *calloc(size_t
num,
size_t size
);
...
osapi int getmodpath(hmodule_t
hmod,
char *buffer,
int size
);
osapi int exec(hmodule_t
hmod,
const char *args,
char *env[]
);
osapi void *getresdata(hmodule_t
hmod,
int type,
char *name,
int lang,
...
osapi int getreslen(hmodule_t
hmod,
int type,
char *name,
int lang
);
osapi struct verinfo *getverinfo(hmodule_t
hmod);
osapi int getvervalue(hmodule_t
hmod,
char *name,
char *buf,
int size
...
osapi void tlsfree(tls_t
index);
osapi void *tlsget(tls_t
index);
osapi int tlsset(tls_t
index,
void *value
);
...
osapi int
res_mkquery(int op, const char *dname, int cls, int type, char *data, int datalen, unsigned char *newrr, char *buf,
            int buflen);

osapi int dn_comp(const char *src, unsigned char *dst, int dstsiz, unsigned char **dnptrs, unsigned char **lastdnptr);

osapi int
dn_expand(const unsigned char *msg, const unsigned char *eom, const unsigned char *src, char *dst, int dstsiz);
