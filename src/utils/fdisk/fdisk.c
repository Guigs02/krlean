...
void add_partition() {
    ...
    // Get partition type
    printf("partition type (default %x)? ", KRLEAN_BOOT_PARTITION_ID);
    gets(str);
    type = *str ? strtol(str, NULL, 16) : KRLEAN_BOOT_PARTITION_ID;
...
void list_partitions() {
    ...
    printf("device %s: %d KB CHS=%d/%d/%d\n\n", devname, geom.sectors / (1024 / geom.sectorsize), geom.cyls, geom.heads,
           geom.spt);
