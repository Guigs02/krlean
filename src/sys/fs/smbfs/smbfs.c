...
int smb_mount(struct fs *fs, char *opts) {
    ...
    get_option(opts, "user", username, sizeof(username), "krlean");
...
int smb_ftruncate(struct file *filp, off64_t size) {
    ...
    rsplen = sizeof(rsp);
    rc = smb_trans(share, TRANS2_SET_FILE_INFORMATION, &req, sizeof(req), &info, sizeof(info), &rsp, &rsplen, NULL,
                   NULL);
    if (rc < 0) return rc;
...
int smb_stat(struct fs *fs, char *name, struct stat64 *buffer) {
    ...
    if (buffer) {
        ...
        rc = smb_trans(share, TRANS2_QUERY_PATH_INFORMATION, &req, sizeof(req) - MAXPATH + strlen(name) + 1, NULL, 0,
                       &dummy, &dummylen, &rspb, &rsplen);
        if (rc < 0) return rc;
    }
...
int smb_opendir(struct file *filp, char *name) {
    ...
    rc = smb_trans(share, TRANS2_FIND_FIRST2, &req, 12 + strlen(req.filename) + 1, NULL, 0, &rsp, &rsplen, dir->buffer,
                   &buflen);
    if (rc < 0) {
        kfree(dir);
        return rc;
...
int smb_readdir(struct file *filp, struct direntry *dirp, int count) {
    ...
    if (dir->fi->filename[0] == '.' &&
        (dir->fi->filename[1] == 0 || (dir->fi->filename[1] == '.' && dir->fi->filename[2] == 0))) {
        dir->entries_left--;
