...
raise_handler(EXCEPTION_RECORD *rec, EXCEPTION_FRAME *frame, CONTEXT *ctxt, EXCEPTION_FRAME **dispatcher) {
...
static EXCEPTION_DISPOSITION
unwind_handler(EXCEPTION_RECORD *rec, EXCEPTION_FRAME *frame, CONTEXT *ctxt, EXCEPTION_FRAME **dispatcher) {
    if (!(rec->ExceptionFlags & (EH_UNWINDING | EH_EXIT_UNWIND))) return ExceptionContinueSearch;
...
return
CreateDirectoryA(fn, lpSecurityAttributes
);
}
...
return (HANDLE)
mkevent(bManualReset, bInitialState
);
}
...
syslog(LOG_WARNING,
"CreateFile(%s): overlapped operation not supported", lpFileName);
errno = EINVAL;
return
INVALID_HANDLE_VALUE;
}

if (dwFlagsAndAttributes & FILE_FLAG_NO_BUFFERING) flags |=
O_DIRECT;
if (dwFlagsAndAttributes & FILE_FLAG_RANDOM_ACCESS) flags |=
O_RANDOM;
if (dwFlagsAndAttributes & FILE_FLAG_SEQUENTIAL_SCAN) flags |=
O_SEQUENTIAL;
if (dwFlagsAndAttributes & FILE_FLAG_DELETE_ON_CLOSE) flags |=
O_TEMPORARY;
if (dwFlagsAndAttributes & FILE_ATTRIBUTE_TEMPORARY) flags |=
O_SHORT_LIVED;
...
return
CreateFileA(fn, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile
);
}
...
t = (time_t)((*(
unsigned __int64
*) lpFileTime) - EPOC) / SECTIMESCALE;

gmtime_r(&t, &tm);

lpSystemTime->
wYear = tm.tm_year + 1900;
lpSystemTime->
wMonth = tm.tm_mon + 1;
lpSystemTime->
wDayOfWeek = tm.tm_wday;
lpSystemTime->
wDay = tm.tm_mday;
lpSystemTime->
wHour = tm.tm_hour;
...
static void fill_find_data(LPWIN32_FIND_DATA fd, char *filename, struct stat64 *statbuf) {
    ...
    *(unsigned __int64
    *) &(fd->ftCreationTime) = (unsigned __int64) statbuf->st_ctime * SECTIMESCALE + EPOC;
    *(unsigned __int64
    *) &(fd->ftLastAccessTime) = (unsigned __int64) statbuf->st_atime * SECTIMESCALE + EPOC;
    *(unsigned __int64
    *) &(fd->ftLastWriteTime) = (unsigned __int64) statbuf->st_mtime * SECTIMESCALE + EPOC;
}
...
fill_find_data(lpFindFileData, dirent
.name, &statbuf);
return (HANDLE)
finddata;
}
...
fill_find_data(lpFindFileData,
".", &statbuf);
return NOFINDHANDLE;
} else {
if (stat64((
char *) lpFileName, &statbuf) < 0) {
errno = ENOENT;
return
INVALID_HANDLE_VALUE;
}
...
fill_find_data(lpFindFileData, base,
&statbuf);
...
&fd, sizeof(WIN32_FIND_DATA) - MAX_PATH - 14);
convert_filename_to_unicode(fd
.cFileName, lpFindFileData->cFileName, MAXPATH);
return
fh;
}
...
size.
QuadPart = fstat64((handle_t)
hFile, NULL);
if (size.QuadPart < 0) return
INVALID_FILE_SIZE;
if (lpFileSizeHigh) *
lpFileSizeHigh = size.HighPart;
return size.
LowPart;
}
...
(
unsigned __int64
) statbuf.st_ctime * SECTIMESCALE + EPOC;
}
...
(
unsigned __int64
) statbuf.st_atime * SECTIMESCALE + EPOC;
}
...
(
unsigned __int64
) statbuf.st_mtime * SECTIMESCALE + EPOC;
}
...
switch (nStdHandle) {
case STD_INPUT_HANDLE:  return (HANDLE)
fdin;
case STD_OUTPUT_HANDLE: return (HANDLE)
fdout;
case STD_ERROR_HANDLE:  return (HANDLE)
fderr;
}
...
strcpy(lpBuffer, get_property(osconfig(),
"win32", "sysdir", "c:\\bin"));
return
strlen(lpBuffer);
}
...
lpSystemInfo->
wProcessorLevel = cpu.cpu_family;

if (cpu.cpu_family == 3) {
lpSystemInfo->
dwProcessorType = 386;
lpSystemInfo->
wProcessorRevision = 0xFFA0;
...
dwProcessorType = 586;
lpSystemInfo->
wProcessorRevision = (cpu.cpu_model << 16) | cpu.cpu_stepping;
}

lpSystemInfo->
dwPageSize = cpu.pagesize;
lpSystemInfo->
dwAllocationGranularity = cpu.pagesize;
lpSystemInfo->
lpMinimumApplicationAddress = (void *) 0x10000;
lpSystemInfo->
lpMaximumApplicationAddress = (void *) 0x7FFFFFFF;
}
...
wSecond = tm.tm_sec;
lpSystemTime->
wMilliseconds = (WORD)(tv.tv_usec / 1000);
}
...
unsigned __int64
) (tv.tv_usec)) * MICROTIMESCALE + EPOC;
}
...
syslog(LOG_DEBUG,
"GetThreadContext(%d) failed", hThread);
return
FALSE;
}
...
lpMaximumComponentLength = MAXPATH - 1;
...
lpBuffer->
dwMemoryLoad = ((mem.physmem_total - mem.physmem_avail) / mem.pagesize) / (mem.physmem_total / mem.pagesize);
lpBuffer->
dwTotalPhys = mem.physmem_total;
lpBuffer->
dwAvailPhys = mem.physmem_avail;
lpBuffer->
dwTotalPageFile = mem.physmem_total;
lpBuffer->
dwAvailPageFile = mem.physmem_avail;
lpBuffer->
dwTotalVirtual = mem.virtmem_total;
lpBuffer->
dwAvailVirtual = mem.virtmem_avail;
}
...
syslog(LOG_DEBUG,
"warning: HeapAlloc only supported for process heap");
return
NULL;
}
...
if (hHeap != PROCESSHEAP) {
syslog(LOG_DEBUG,
"warning: HeapFree only supported for process heap");
return
FALSE;
}
...
TRACE("MoveFileA");
if (
rename(lpExistingFileName, lpNewFileName
) < 0) return
FALSE;
...
if (!lpPreviousCount)
lpPreviousCount = &dummy;
*
lpPreviousCount = semrel((handle_t)
...
unwind(endframe, eip, rec, retval,
&ctxt);
}
...
syslog(LOG_DEBUG,
"warning: SetFileAttributesA(%s,%08x) not implemented, ignored", lpFileName, dwFileAttributes);
return
TRUE;
}
...
return
SetFileAttributesA(fn, dwFileAttributes
);
}
...
offset.
LowPart = lDistanceToMove;
offset.
HighPart = *lpDistanceToMoveHigh;

retval.
QuadPart = lseek64((handle_t)
hFile, offset.QuadPart, dwMoveMethod);
...
times.
ctime = (time_t)(((*(__int64 * )
lpCreationTime) - EPOC) / SECTIMESCALE);
} else {
times.
ctime = -1;
}
...
times.
actime = (time_t)(((*(__int64 * )
lpLastAccessTime) - EPOC) / SECTIMESCALE);
} else {
times.
actime = -1;
}
...
times.
modtime = (time_t)(((*(__int64 * )
lpLastWriteTime) - EPOC) / SECTIMESCALE);
} else {
times.
modtime = -1;
}
...
syslog(LOG_WARNING,
"kernel32: incomplete context in SetContext, flags %lx", lpContext->ContextFlags);
}
...
tm.
tm_year = lpSystemTime->wYear - 1900;
tm.
tm_mon = lpSystemTime->wMonth - 1;
tm.
tm_mday = lpSystemTime->wDay;
tm.
tm_hour = lpSystemTime->wHour;
tm.
tm_min = lpSystemTime->wMinute;
tm.
tm_sec = lpSystemTime->wSecond;
...
unsigned __int64
) (lpSystemTime->wMilliseconds) * 1000) * MICROTIMESCALE + EPOC;
return
TRUE;
}
...
for (
n = 0;
n < MAX_VADS; n++) if (vads[n].addr == NULL) break;
if (n != MAX_VADS) {
vads[n].
addr = addr;
vads[n].
...
for (
n = 0;
n < MAX_VADS; n++) if (vads[n].addr == lpAddress) break;
if (n != MAX_VADS) {
vads[n].
addr = NULL;
dwSize = vads[n].size;
...
syslog(LOG_WARNING,
"warning: vad not found for VitualFree");
}
}
...
lpBuffer->
BaseAddress = tib->stacklimit;
lpBuffer->
RegionSize = (char *) tib->stacktop - (char *) tib->stacklimit;
lpBuffer->
AllocationBase = tib->stackbase;
lpBuffer->
...
// Return dummy result
lpBuffer->
BaseAddress = (void *) ((unsigned long) lpAddress & ~(PAGESIZE - 1));
lpBuffer->
Protect = PAGE_READWRITE;
}
...
if (bWaitAll) {
rc = waitall((handle_t * )
lpHandles, nCount, dwMilliseconds);
} else {
rc = waitany((handle_t * )
lpHandles, nCount, dwMilliseconds);
}
