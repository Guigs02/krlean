...
sockapi int __stdcall WSAStartup(WORD wVersionRequested, LPWSADATA lpWSAData) {
    ...
    if (lpWSAData) {
        ...
        strcpy(lpWSAData->szDescription, "krlean Winsock 1.1");
...
sockapi int
__stdcall winsock_sendto(SOCKET s, const char *buf, int len, int flags, const struct sockaddr *to, int tolen) {
    TRACE("sendto");
    return sendto(s, buf, len, flags, to, tolen);
}

sockapi int __stdcall winsock_select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
