...
static int pe_find_import(CCState *s1, const char *symbol) {
    char buffer[200];
    const char *s;
    int sym_index, n = 0;
    do {
    ...
    for (i = 0; i < pe->sec_count; ++i) {
        ...
        switch (si->cls) {
            case sec_text:
                ...
                pe_opthdr.AddressOfEntryPoint =
                        ((Elf32_Sym *) symtab_section->data)[pe->start_sym_index].st_value - pe->imagebase;
                break;
...
static int pe_assign_addresses(struct pe_info *pe) {
    ...
    pe->sec_info = cc_mallocz(o * sizeof(struct section_info));
    addr = pe->imagebase + 1;
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                // cc_realloc might have altered sym's address
                sym = (Elf32_Sym *) symtab_section->data + sym_index;
...
int pe_load_def_file(CCState *s1, int fd) {
    DLLReference *dllref;
...
static void pe_add_runtime_ex(CCState *s1, struct pe_info *pe) {
    ...
    if (find_elf_sym(symtab_section, "_WinMain@16")) {
        pe_type = PE_GUI;
    } else if (s1->output_type == CC_OUTPUT_DLL) {
        pe_type = PE_DLL;
        // Need this for 'ccelf.c:relocate_section()'
        s1->output_type = CC_OUTPUT_EXE;
    } else {
        pe_type = PE_EXE;
    }
    ...
    cc_get_symbol_err(s1, start_symbol);

    if (s1->nostdlib == 0) {
        cc_add_library(s1, "c");
        cc_add_library(s1, "os");
    }
...
void pe_add_runtime(CCState *s1) {
    pe_add_runtime_ex(s1, NULL);
}

int pe_output_file(CCState *s1, const char *filename) {
    int ret;
    struct pe_info pe;
    int i;
