        ...
        while (tok != ')') {
            ...
            switch (t) {
                ...
                case TOK_SECTION2:
                    ...
                    ad->section = find_section(cc_state, (char *) tokc.cstr->data);
                    next();
                    skip(')');
                    break;
                ...
                case TOK_UNUSED1:
                case TOK_UNUSED2:
                    // Currently, no need to handle it because cc does not track unused objects
                    break;
            ...
            while (tok != '}') {
                ...
                while (1) {
                    ...
                    if (ad.aligned) {
                        ...
                        align = 1;
                    } else if (*cc_state->pack_stack_ptr) {
                        if (align > *cc_state->pack_stack_ptr) align = *cc_state->pack_stack_ptr;
                    }
...
void unary(void) {
    ...
    switch (tok) {
        ...
        case TOK_STR:
            ...
            t = VT_BYTE;
        str_init:
            if (cc_state->warn_write_strings) t |= VT_CONSTANT;
            type.t = t;
            mk_pointer(&type);
            type.t |= VT_ARRAY;
            memset(&ad, 0, sizeof(AttributeDef));
        ...
        default:
            ...
            if (!s) {
                ...
                if (cc_state->warn_implicit_function_declaration) {
                ...
                if ((type.t & (VT_INLINE | VT_STATIC)) == (VT_INLINE | VT_STATIC)) {
                    ...
                    if (!cur_text_section) {
                        if (!cc_state->nofll) {
                            ...
                            cur_text_section = find_section(cc_state, section_name.data);
                        } else {
                            cur_text_section = text_section;
                        }
                    }
...
void preprocess_init(CCState *s1) {
...
void cc_undefine_symbol(CCState *s1, const char *sym) {
    TokenSym *ts;
    Sym *s;
...
CCState *cc_new(void) {
    ...
    s = cc_mallocz(sizeof(CCState));
    if (!s) return NULL;
    cc_state = s;
    s->output_type = CC_OUTPUT_EXE;
    ...
    // Standard defines
    cc_define_symbol(s, "__STDC__", NULL);
    cc_define_symbol(s, "__STDC_VERSION__", "199901L");
    cc_define_symbol(s, "__i386__", NULL);
    cc_define_symbol(s, "KRLEAN", NULL);

    // Tiny C specific defines
    cc_define_symbol(s, "__TINYC__", NULL);
    cc_define_symbol(s, "_CC_VER", "\"" CC_VERSION "\"");

    // Tiny C & gcc defines
    cc_define_symbol(s, "__SIZE_TYPE__", "unsigned int");
    cc_define_symbol(s, "__PTRDIFF_TYPE__", "int");
    cc_define_symbol(s, "__WCHAR_TYPE__", "unsigned short");
    cc_define_symbol(s, "_INTEGRAL_MAX_BITS", "64");
    cc_define_symbol(s, "_CC_PLATFORM", "\"" CC_PLATFORM "\"");
    ...
    s->dynsymtab_section = new_symtab(s, ".dynsymtab", SHT_SYMTAB, SHF_PRIVATE, ".dynstrtab", ".dynhashtab",
                                      SHF_PRIVATE);
    s->alacarte_link = 1;
...
void cc_delete(CCState *s1) {
    ...
    for (i = 0; i < n; i++) cc_free(table_ident[i]);
    cc_free(table_ident);
...
int cc_add_include_path(CCState *s1, const char *pathname) {
    char *pathname1;

    pathname1 = cc_strdup(pathname);
    dynarray_add((void ***) &s1->include_paths, &s1->nb_include_paths, pathname1);
    return 0;
}

int cc_add_sysinclude_path(CCState *s1, const char *pathname) {
    char *pathname1;
...
int cc_add_file_ex(CCState *s1, const char *filename, int flags) {
    const char *ext;
    Elf32_Ehdr ehdr;
    int fd, ret;
    ...
    ext = cc_fileextension(filename);
    if (ext[0]) ext++;

    // Open the file
    saved_file = file;
    file = cc_open(s1, filename);
    if (!file) {
    ...
    if (flags & AFF_PREPROCESS) {
        ret = cc_preprocess(s1);
    } else if (!ext[0] || !strcmp(ext, "c")) {
        // C file assumed
        ret = cc_compile(s1);
    } else if (!strcmp(ext, "S")) {
        // Preprocessed assembler
        ret = cc_assemble(s1, 1);
    } else if (!strcmp(ext, "s")) {
        // Non-preprocessed assembler
        ret = cc_assemble(s1, 0);
        ...
        if (ehdr.e_ident[0] == ELFMAG0 && ehdr.e_ident[1] == ELFMAG1 && ehdr.e_ident[2] == ELFMAG2 &&
            ...
            if (ehdr.e_type == ET_REL) {
                ret = cc_load_object_file(s1, fd, 0);
            } else if (ehdr.e_type == ET_DYN) {
                ret = cc_load_dll(s1, fd, filename, (flags & AFF_REFERENCED_DLL) != 0);
            } else {
...
int cc_add_file(CCState *s, const char *filename) {
    return cc_add_file_ex(s, filename, AFF_PRINT_ERROR);
}

int cc_add_library_path(CCState *s, const char *pathname) {
    char *pathname1;
...
int cc_add_dll(CCState *s, const char *filename, int flags) {
    ...
    for (i = 0; i < s->nb_library_paths; i++) {
        ...
        if (cc_add_file_ex(s, buf, flags) == 0) return 0;
    }
    return -1;
}
...
int cc_set_output_type(CCState *s, int output_type) {
    char buf[1024];
    ...
    if (!s->nostdinc) {
        ...
        cc_add_sysinclude_path(s, buf);
    }

    if (s->char_is_unsigned) {
        cc_define_symbol(s, "__CHAR_UNSIGNED__", NULL);
    }
